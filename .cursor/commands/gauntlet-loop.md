...
- **Critic uses fresh context.** Trigger: assigning critique. Wrong: critic sees the full build transcript and rationalizations. Correct: give the critic only the artifact, the reference pack (or both unlabeled), and the part bar. Reason: build history contaminates comparison.
