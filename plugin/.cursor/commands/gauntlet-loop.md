...
Run a Gauntlet Loop: take a GOAL and a REAL-WORLD EQUIVALENT as the quality bar, decompose into independent parts, assign each part to a specialist builder, and gate every part behind a separate fresh-context critic that inspects the artifact and compares it against the reference (blind side-by-side when possible). A part passes only when the critic judges it better than the reference; otherwise the critic names the largest specific gap and the part returns to the builder. Builders never evaluate their own work. Full workflow: `.cursor/skill-contracts/gauntlet-loop/SKILL.md` (user install: `~/.cursor/skill-contracts/gauntlet-loop/SKILL.md`).

## Defaults

| Setting | Default |
|---------|---------|
| Intake | GOAL and REAL-WORLD EQUIVALENT required |
...
2. **Execute** phases in order (Intake → Decompose → Build → Critique → Loop → Deliver).
...
- **Pass only if better than the reference.** Trigger: critic returns a verdict. Wrong: passing on "good enough" or "close". Correct: pass only when the generated artifact is better than the REAL-WORLD EQUIVALENT; otherwise return the largest specific gap. Reason: the bar is beat-the-example, not polish-until-tired.
- **Largest specific gap, not vague feedback.** Trigger: critic finds the generated work inferior. Wrong: "improve polish" or "make it better". Correct: name the single largest concrete gap versus the reference. Reason: vague feedback wastes the next builder round.
- **Blind side-by-side when possible.** Trigger: comparing artifact and reference. Wrong: telling the critic which item is generated when unlabeled comparison is feasible. Correct: present them side by side without labels when possible; if not (e.g. live product reference), state the comparison was not blind. Reason: labeled comparison biases the critic toward the known reference.
