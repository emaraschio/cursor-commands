...
- **User plugin packaging**: `.cursor-plugin/plugin.json` and `.cursor-plugin/marketplace.json` bundle `.cursor/commands` and `.cursor/skills` for install via **Customize → Plugins** (GitHub URL or local folder; account sync including Cursor iOS). [docs/PLUGIN.md](docs/PLUGIN.md) documents install, mobile smoke, and local dev symlink. Validator checks manifest paths and semver.
