...
- **`/design-agent-loop`**: slash command and skill to design a reusable agent loop as a graph before running it (TASK plus COMPLETION CRITERIA; mermaid node graph; context / tools / expected output / evidence per node; labeled transition triggers; retry caps and budget ceilings; human approval gates on irreversible nodes; manual carve-outs for unverifiable steps; untrusted input fenced as data so embedded directives cannot steer the loop; user constraints carried into the graph as forbidden transitions, gates, or fail signals; five eval cases sized to volume; one first bottleneck; two-step handshake, one test case on `execute now`); eval ship gate **A, S, R** (19 cases); catalog 43 → 44 commands/skills.
...
- **`/gauntlet-loop`**: clarify reference vs reference pack in critic context; add live IDE smoke prompt and checklist under `eval/README.md` (linked from VERIFICATION).
